# 0) Librairies -------------------------------------------------------------
pacman::p_load(tidyverse, ggforce, ggnewscale)
...
# Version avec un élément = 1 article
# df_counts <- df_articles %>%
#   rename(
...
# Version avec un élément = 1 réseau
...
df_unique <- df_networks %>%
  ...
  summarise(
    ...
    # Conserver toutes les valeurs distinctes de indirect_transfer_max_delay, séparées par des virgules
    max_delay_arrangé = paste(unique(max_delay_arrangé), collapse = ", "),
    .groups = "drop"
  )

# Pour avoir le délai max par network
...
# Pour chaque ligne, prendre le max (en années) parmi les valeurs séparées par des virgules
df_unique <- df_unique %>%
  mutate(
...
# 3) Calcul des angles pour chaque anneau -----------------------------------
...
# 4) Construction du sunburst + légende hiérarchique ------------------------
p <- ggplot()
...
# 2. Ajouter une slide blanche
doc <- add_slide(doc, layout = "Blank", master = "Office Theme")

# 3. Convertir le ggplot en graphique vectoriel éditable
p_dml <- dml(ggobj = p)   # conversion via rvg

# 4. Insérer dans la diapositive
